package as3flatbuffers
{
    ...
    public final class Builder
    {
        ...
        public function get capacity():uint { return bytes.length; }
        ...
        {
            if (fields == null) throw new Error("No table is open");
            putInt32(0);
            const objectOffset:uint = offset;
            const objectSize:uint = objectOffset - tableStart;
            if (objectSize > 65535) throw new RangeError("Table is too large");
            var count:uint = fields.length;
            while (count && fields[count - 1] == 0) count--;
        ...
        private function get offset():uint { return bytes.length - space; }
        ...
        private function checkSlot(slot:uint):void
        {
            ...
            if (fields[slot]) throw new Error("Field was already written");
        }
        ...
        private function prepare(alignment:uint):void
        {
            ...
            while (space < padding + alignment)
            {
                ...
                if (used) grown.writeBytes(bytes, space, used);
